// src\app\components\ArticleEdit.tsx
"use client";
...
const ArticleEdit: React.FC<{ articleId: number }> = ({ articleId }) => {
  ...
  useEffect(() => {
    const fetchArticleData = async () => {
      try {
        ...
        if (!response.ok) throw new Error('خطا در بارگذاری اطلاعات مقاله');
        ...
        alert('خطا در بارگذاری اطلاعات مقاله');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('خطا در به‌روزرسانی مقاله: ' + (error as Error).message);
    } finally {
