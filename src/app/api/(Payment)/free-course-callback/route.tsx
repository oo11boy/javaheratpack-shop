...
export async function POST(req: NextRequest) {
  try {
    ...
    // تولید orderId مشابه پرداخت
    const timestamp = Date.now();
    const orderId = `ORDER-${timestamp}-${decoded.id}-${courseIds.join(",")}`;

    // ثبت دوره‌ها در دیتابیس
    ...
    // تولید redirectUrl برای callback
    const purchaseDate = new Date(timestamp).toLocaleString("fa-IR");
    const redirectUrl = `${BASE_URL}/api/callback?success=1&trackId=FREE-${timestamp}&orderId=${orderId}&courseIds=${courseIds.join(",")}&purchaseDate=${encodeURIComponent(purchaseDate)}&totalAmount=0`;

    return NextResponse.json({ redirectUrl });
  } catch (error) {
    console.error("Free course callback error:", error);
